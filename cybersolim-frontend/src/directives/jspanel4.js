...
const jspanel4 = {
    ...
    inserted(el, binding, vnode) {
        ...
        let defaultOption = {
            ...
            content: el,
            onwindowresize: true,
            // 必须设置 index.vue 的 body为 100vh（即整个可视区域的高度） 或大于面板高度，否则拖动时会出现面板消失的问题
            dragit: { containment: 0 }
        };
        // combine options and defaultOption with an object
        let option = Object.assign({}, defaultOption, options);
        // vnode.jsPanelOption = option;
        if (show) {
            // 记录panel，用于close等操作. 使用vnode来共享数据。Vue官方推荐使用dataset来在钩子间共享数据(function不适用)
            vnode.panel = null;
    ...
    update(el, binding, vnode, oldVnode) {
        ...
        //vnode.panel.parentElement is not null
    },
